...
public class PlayerMovement : MonoBehaviour {
    ...
    private void Look() {
        ...
        //playerMesh.transform.localRotation = Quaternion.Euler(0, turn.x, 0);

        if (Math.Abs(wallRunCameraTilt) < wallRunCameraTilt && isWallRunning && isWallRight)
            wallRunCameraTilt += Time.deltaTime * maxWallRunCameraTilt * 2;
        if (Math.Abs(wallRunCameraTilt) < maxWallRunCameraTilt && isWallRunning && isWallleft)
            wallRunCameraTilt -= Time.deltaTime * maxWallRunCameraTilt * 2;

        // Tilt the camera back again
